...
class QdrantVectorStore:
    ...
    def upsert_chunks(
        self, file_path: str, chunks: list[AstChunk], vectors: list[list[float]]
    ) -> None:
    ...
    def delete_by_file(self, file_path: str) -> None:
        """해당 파일의 기존 point를 전부 지운다.

        point id는 (파일, 시작/끝 라인, ...)로 결정되므로, 파일을 재인덱싱할 때
        ...
        self._client.delete(
            ...
            points_selector=Filter(
                must=[FieldCondition(key="filePath", match=MatchValue(value=file_path))]
            ),
        )

    def search(self, query_vector: list[float], *, limit: int = 5) -> list[ChunkSearchResult]:
